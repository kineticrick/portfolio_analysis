...
from libraries.db.sql import (create_sectors_history_table_sql, insert_update_sectors_history_sql, 
                           insert_ignore_sectors_history_sql, read_sectors_history_query, 
...
class SectorHistoryHandler(BaseHistoryHandler):
    ...
    def __init__(self) -> None: 
        ...
        sector histories from DB. (Date, symbol, quantity, price, value)
        
        If 'symbols' is empty, initialize object with all sector histories from DB.
        """
        super().__init__()

    def set_history(self, start_date: str=None, overwrite: bool=False) -> None:
        """
        For all symbols in self.symbols, update DB with sector history info 
        ...
        sectors_historical_data_df = \
            gen_aggregated_historical_value(self.symbols, cadence='daily', 
    ...
    def get_history(self) -> pd.DataFrame:
        """
        For all symbols in self.symbols, get sector history from DB into dataframe
        ...
        query = read_sectors_history_query + symbols_str
        history_df = mysql_to_df(query, read_sectors_history_columns, dbcfg, cached=True)
        return history_df